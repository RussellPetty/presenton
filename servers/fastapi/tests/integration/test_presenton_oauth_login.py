...
def test_cloud_generation_proxy_uses_server_side_presenton_token(
    monkeypatch, tmp_path
):
    monkeypatch.setenv("USER_CONFIG_PATH", str(tmp_path / "userConfig.json"))
    client, engine, session_maker = _build_client(tmp_path)
    ...
    async def add_identity():
        ...
        async with session_maker() as session:
            ...
            await store_presenton_credentials(
                ...
                user_id=user.id,
                issuer="https://accounts.presenton.test",
                access_token="pt_access_private",
                refresh_token="pt_refresh_private",
                scope="presenton:api profile:read",
                expires_in=3600,
            )

    asyncio.run(add_identity())
    captured = {}

    async def request_cloud(_session, **kwargs):
        captured.update(kwargs)
        return _response(
            200,
            {
                "presentation_id": "cloud-deck-id",
                "path": "https://accounts.presenton.test/deck.pptx",
                "edit_path": "https://presenton.test/presentation?id=cloud-deck-id",
                "credits_consumed": 12,
            },
        )

    monkeypatch.setattr(presenton_oauth, "request_presenton_cloud", request_cloud)
    generated = client.post(
        "/api/v1/auth/presenton/cloud/presentation/generate",
        json={"content": "Build a cloud security presentation", "n_slides": 8},
    )

    assert generated.status_code == 200
    assert generated.json()["presentation_id"] == "cloud-deck-id"
    assert captured["path"] == "/api/v3/presentation/generate"
    assert captured["json"]["n_slides"] == 8
    assert "access_token" not in captured["json"]
    asyncio.run(engine.dispose())
