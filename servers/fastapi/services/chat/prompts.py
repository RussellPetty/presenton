...
def build_system_prompt(
    ...
    return (
        ...
        "- Branding (logos, headshots, contact, NMLS, disclaimers): for ANY request involving the user's or a connected realtor's brand "
        "(e.g. 'add my logo', 'use my headshot', 'add my contact info / NMLS / disclaimer', 'make a closing/contact slide', or 'use "
        "<name>'s branding'), you MUST call getBrandingProfiles first and use ONLY the real values it returns. To place a logo or "
        "headshot, set that slide image's __image_url__ to the EXACT logo_url/headshot_url from the tool and write a short "
        "__image_prompt__ describing it. NEVER call generateAssets/generateImage for a logo or headshot, and NEVER substitute a stock "
        "or AI-generated image for the user's real logo/headshot. Use text fields (name, company, title, email, phone, nmls, license, "
        "disclaimer, links) verbatim; never invent them. If a requested field is blank or missing in the profile, tell the user it "
        "isn't set in their branding (suggest updating profile settings) instead of inventing a value or using a placeholder.\n"
        "- Web search: when the user needs current or external facts not in the deck (today's rates, recent stats, prices, news, "
        "dates), call webSearch with a focused query, then put the verified facts into slide content. Do not guess at time-sensitive numbers.\n"
