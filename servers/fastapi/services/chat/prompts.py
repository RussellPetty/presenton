...
def build_system_prompt(
    ...
    return (
        ...
        "\n"
        "Images — CRITICAL (read first)\n"
        "- A logo, a headshot, or ANY image the user provides/uploads or refers to as 'mine', 'my brand/logo/headshot', "
        "a named connected realtor/partner's ('Melissa's logo', 'cobrand with Melissa Smith'), or 'this image' must use the "
        "REAL image. Get its url from getBrandingProfiles (the user's or the matching partner's logo_url/headshot_url) or "
        "getMyImages, and put that EXACT url into the slide's image __image_url__ in your saveSlide content.\n"
        "- NEVER call generateAssets/generateImage for a logo, headshot, or user-provided image, and NEVER use a stock, "
        "AI-generated, or placeholder image in its place. Using a generated/stock image for a real logo/headshot/user image "
        "is a hard error. Only generate images for purely decorative/illustrative content you are inventing.\n"
        "\n"
        ...
        "- Co-branding: for 'cobrand this with <name>' / 'add <name>'s branding', call getBrandingProfiles, find the entry in "
        "`partners` whose name matches <name>, and put BOTH the user's and that partner's real branding on the slide — each one's "
        "exact logo_url/headshot_url in an __image_url__, plus their names/titles/companies/contact. If no partner matches that name, "
        "say so and ask the user to confirm the realtor instead of guessing or generating an image.\n"
        ...
        "- User images: when the user attaches an image or asks to use 'this image/photo', an image they uploaded, or one of their own "
        "images, call getMyImages and set the target slide image's __image_url__ to the chosen image's url (write a short __image_prompt__). "
        "Prefer attached_to_this_message for 'this image'. Do NOT generateAssets/generateImage or stock-search when the user wants to use their own image.\n"
        ...
        "- Generate required assets in batch with generateAssets before saving — but ONLY for decorative/illustrative images "
        "you are inventing. NEVER for logos, headshots, or user-provided/brand images; place those exact urls "
        "(from getBrandingProfiles/getMyImages) directly into __image_url__ instead.\n"
