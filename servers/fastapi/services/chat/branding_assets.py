...
# Slide content comes in two shapes and both reach this code.
#
#   v1-standard : image slots are dicts carrying `__image_url__` / `__image_prompt__`
#   v2-standard : slide.ui holds konva elements, where an image element is
#                 {"type": "image", "data": <url>, "prompt": <text>, ...}
#
# utils/process_slides.py switches on the same distinction (see
# `_asset_url_key`), and it regenerates any slot that still has a prompt — which
# is exactly what we suppress for real brand assets.
_V1_URL_KEY = "__image_url__"
_V1_PROMPT_KEY = "__image_prompt__"
_V2_URL_KEY = "data"
_V2_PROMPT_KEY = "prompt"


def _slot_keys(slot: dict[str, Any]) -> tuple[str, str]:
    """Return the (url_key, prompt_key) pair this slot uses."""
    if _V1_URL_KEY in slot or _V1_PROMPT_KEY in slot:
        return _V1_URL_KEY, _V1_PROMPT_KEY
    return _V2_URL_KEY, _V2_PROMPT_KEY


def _is_v2_image_element(node: dict[str, Any]) -> bool:
    # `is_icon` distinguishes a real image from an icon slot; icons are matched
    # by query elsewhere and must not be replaced with a logo.
    return node.get("type") == "image" and not node.get("is_icon")
...
def _iter_image_slots(node: Any) -> Iterator[dict[str, Any]]:
    ...
    if isinstance(node, dict):
        if (
            _V1_PROMPT_KEY in node
            or _V1_URL_KEY in node
            or _is_v2_image_element(node)
        ):
            yield node
...
def apply_brand_assets_to_content(
    ...
    for slot in _iter_image_slots(content):
        url_key, prompt_key = _slot_keys(slot)

        current_url = slot.get(url_key)
        if isinstance(current_url, str) and current_url.strip() in known_urls:
            if slot.pop(prompt_key, None) is not None:
                changed += 1
            continue

        # A v2 element names its slot, so "Agent headshot" is a signal even when
        # the model wrote no prompt at all.
        prompt_lower = " ".join(
            str(slot.get(key) or "") for key in (prompt_key, "name")
        ).lower().strip()
        if not prompt_lower:
            continue
        ...
        if isinstance(url, str) and url.strip():
            slot[url_key] = url.strip()
            slot.pop(prompt_key, None)
            changed += 1
