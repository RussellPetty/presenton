...
export type PresentonCloudStatus = {
  enabled: boolean;
  linked: boolean;
...
export type PresentonCloudGenerationResult = {
...
export type PresentonCloudTask = {
  id: string;
  status: "pending" | "completed" | "error";
  message?: string | null;
  data?: PresentonCloudGenerationResult | null;
  error?: { detail?: string } | null;
};

export class PresentationGenerationApi {
  static async getPresentonCloudStatus(): Promise<PresentonCloudStatus> {
    ...
    return await ApiResponseHandler.handleResponse(
      response,
      "Failed to check Presenton cloud connection"
    );
  }

  static async uploadPresentonCloudDocuments(
    documents: File[]
  ): Promise<string[]> {
    const formData = new FormData();
    documents.forEach((document) => formData.append("files", document));
    const response = await fetch(
      getApiUrl(`/api/v1/auth/presenton/cloud/files/upload`),
      {
        method: "POST",
    ...
    return await ApiResponseHandler.handleResponse(
      response,
      "Failed to upload documents to Presenton cloud"
    );
  }

  static async startPresentonCloudGeneration(
    payload: Record<string, unknown>
  ): Promise<PresentonCloudTask> {
    const response = await fetch(
      getApiUrl(`/api/v1/auth/presenton/cloud/presentation/generate/async`),
      {
        method: "POST",
        headers: getHeader(),
    ...
    return await ApiResponseHandler.handleResponse(
      response,
      "Failed to start Presenton cloud generation"
    );
  }

  static async getPresentonCloudTask(taskId: string): Promise<PresentonCloudTask> {
    const response = await fetch(
      getApiUrl(
        `/api/v1/auth/presenton/cloud/async-task/status/${encodeURIComponent(taskId)}`
      ),
      {
        method: "GET",
        headers: getHeader(),
    ...
    return await ApiResponseHandler.handleResponse(
      response,
      "Failed to check Presenton cloud generation"
    );
  }
